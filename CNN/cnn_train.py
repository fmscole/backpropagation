...
from CNN.relu import Relu,sigmoid,Relu_Sigmoid
    ...
    for i in range(50):
        ...
        for j in range(batch_size):
            if np.argmax(sf.softmax[j]) == np.argmax(labs[j]):
                train_acc += 1
        print ("%d /%d   train_acc: %.4f  " % (i,10000//batch_size//10,train_acc / ((i+1) * batch_size)))
    print (" train_acc: %.4f  " % (train_acc /  (10000//batch_size//10*batch_size)))


def forward(imgs, labs,training=True):
    conv1_out = conv1.forward(imgs)
    bn1_out=bn1.forward(conv1_out, axis=-1,training=training)
    relu1_out = relu1.forward(bn1_out)
    pool1_out = pool1.forward(relu1_out)
    conv2_out = conv2.forward(pool1_out)
    bn2_out = bn2.forward(conv2_out, axis=-1,training=training)
